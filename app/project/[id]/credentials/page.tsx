...
type SortField = "username" | "password" | "source_host" | "obtained_via" | "valid_for"
type SortDirection = "asc" | "desc"

export default function CredentialsPage() {
  const { currentProject, addCredential, updateCredential, deleteCredential } = useProject()
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Manage credentials in your project</p>
        </div>
          ...
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>Add a new credential to your project</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewCredential({ ...newCredential, source_host: value })}
                  >
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewCredential({ ...newCredential, obtained_via: e.target.value })}
                ...
                <div className="grid gap-2">
                  ...
                  <div className="border rounded-md p-4 max-h-40 overflow-y-auto">
                    {currentProject.hosts.map((host) => (
                      <div key={host.host_id} className="flex items-center space-x-2 mb-2">
                        <Checkbox
                          id={`host-${host.host_id}`}
                          checked={(newCredential.valid_for || []).includes(host.host_id)}
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setNewCredential({
                                ...newCredential,
                                valid_for: [...(newCredential.valid_for || []), host.host_id],
                              })
                            } else {
                              setNewCredential({
                                ...newCredential,
                                valid_for: (newCredential.valid_for || []).filter((id) => id !== host.host_id),
                              })
                            }
                          }}
                        />
                        <Label htmlFor={`host-${host.host_id}`} className="text-sm">
                          {host.hostname || host.ip_address}
                        </Label>
                      </div>
                    ))}
      ...
      {sortedCredentials.length === 0 ? (
        <div className="text-center py-10">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Add your first credential to get started.</p>
        </div>
      ) : viewMode === "card" ? (
          ...
          {sortedCredentials.map((credential) => (
            <Card key={credential.cred_id}>
              <CardHeader>
                ...
                <CardDescription>
                  Source: {getHostNameById(credential.source_host)} via {credential.obtained_via}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="font-mono text-sm">
                        {showPasswords[credential.cred_id] ? credential.password : "••••••••••••"}
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-6 w-6 p-0"
                        onClick={() => togglePasswordVisibility(credential.cred_id)}
                      >
                      ...
                      <Button
                        ...
                        size="sm"
                        className="h-6 w-6 p-0"
                        onClick={() => copyToClipboard(credential.password, credential.cred_id)}
                      >
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {credential.valid_for.map((hostId) => (
                          <Badge key={hostId} variant="outline" className="flex items-center gap-1">
              ...
              <CardFooter className="flex justify-between">
                <Dialog
                  open={isEditOpen && editingCredential?.cred_id === credential.cred_id}
                  onOpenChange={(open) => {
                    setIsEditOpen(open)
                ...
                >
                  <DialogTrigger asChild onClick={() => setEditingCredential(credential)}>
                      ...
                      <div className="grid gap-4 py-4">
                        ...
                        <div className="grid gap-2">
                          ...
                          <div className="flex gap-2">
                            <Input
                              id="edit_password"
                              type={showPasswords[editingCredential.cred_id] ? "text" : "password"}
                            ...
                            <Button
                              variant="outline"
                              size="icon"
                              onClick={() => togglePasswordVisibility(editingCredential.cred_id)}
                            >
                        ...
                        <div className="grid gap-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              setEditingCredential({ ...editingCredential, source_host: value })
                            }
                          >
                        ...
                        <div className="grid gap-2">
                          <Label htmlFor="edit_obtained_via">Obtained Via</Label>
                          <Input
                            id="edit_obtained_via"
                            value={editingCredential.obtained_via}
                            onChange={(e) =>
                              setEditingCredential({ ...editingCredential, obtained_via: e.target.value })
                            }
                          />
                        </div>
                        <div className="grid gap-2">
                            ...
                            {currentProject.hosts.map((host) => (
                              <div key={host.host_id} className="flex items-center space-x-2 mb-2">
                                <Checkbox
                                  ...
                                  checked={editingCredential.valid_for.includes(host.host_id)}
                                  onCheckedChange={(checked) => {
                                    if (checked) {
                                      setEditingCredential({
                                        ...editingCredential,
                                        valid_for: [...editingCredential.valid_for, host.host_id],
                                      })
                                    } else {
                                      setEditingCredential({
                                        ...editingCredential,
                                        valid_for: editingCredential.valid_for.filter((id) => id !== host.host_id),
                                      })
                                    }
                                  }}
                                />
                                <Label htmlFor={`edit-host-${host.host_id}`} className="text-sm">
                                  {host.hostname || host.ip_address}
                                </Label>
                              </div>
                            ))}
                ...
                <Dialog
                  open={isDeleteOpen && credentialToDelete === credential.cred_id}
                  onOpenChange={(open) => {
                    setIsDeleteOpen(open)
                    if (!open) setCredentialToDelete(null)
                  }}
                >
                  <DialogTrigger asChild onClick={() => setCredentialToDelete(credential.cred_id)}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer" onClick={() => handleSort("source_host")}>
                  <div className="flex items-center">Source Host {getSortIcon("source_host")}</div>
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort("obtained_via")}>
                  <div className="flex items-center">Obtained Via {getSortIcon("obtained_via")}</div>
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort("valid_for")}>
                  <div className="flex items-center">Valid For {getSortIcon("valid_for")}</div>
                </TableHead>
            ...
            <TableBody>
              {sortedCredentials.map((credential) => (
                <TableRow key={credential.cred_id}>
                  ...
                  <TableCell>{getHostNameById(credential.source_host)}</TableCell>
                  <TableCell>{credential.obtained_via}</TableCell>
                  <TableCell>
                    {credential.valid_for.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {credential.valid_for.slice(0, 2).map((hostId) => (
                          <Badge key={hostId} variant="outline" className="text-xs">
                            {getHostNameById(hostId)}
                          </Badge>
                        ))}
